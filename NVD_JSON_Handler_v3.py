...
json_summary_file = 'vulns_info_v3.csv'
cwe_data_file = 'CWE Data/Combined_CWE.csv'
json_feed_url = 'https://nvd.nist.gov/vuln/data-feeds#JSON_FEED'
enc = 'utf-8' # enc = 'utf-16', enc = 'iso-8859-15', enc = 'cp437'
# making data frame from csv file 

cwe_data = pd.read_csv(cwe_data_file) # load cwe data file in dataframs

# 1. Download files...
# Downloads files provided under the NVD JSON Feeds section: https://nvd.nist.gov/vuln/data-feeds#JSON_FEED 
# TODO: 
# Update method to download nvdcve-1.1-modified and nvdcve-1.1-recent file
def download_json_files(feed_url):
    print('Step 1: Obtaining NVD JSON files from NVD Website.\n['+json_feed_url+']')
    # Code at the moment only downloads the yearly NVD JSON files (2002-2020)
...
# TODO: 
# Check for already existing files, overwrite them.
def extract_and_remove_json():
    print('Step 2: Extracting NVD JSON files.')
...
# 3. Prepare data from downloaded JSON files...
...
def read_and_summarize():
    ...
    with open(json_summary_file, 'w', newline='') as file:
        writer = csv.writer(file)
        # write the header...
        writer.writerow(['CVE-ID','Lang', 'CWE_ID', 'CWE_Desc', 'CWE_Plat', 'CWE_Af_Res', 'Severity', 'CVSS_V2', 'CVSS_V3', 'Vul_Access_Vector', 'UserInteractionReq', 'OS', 'SW', 'Ports', 'PublishedDate','LastModified','Description', 'URL&Tags'])
        ...
        for root, dirs, files in os.walk(json_files_path + '/', topdown=False):
            for name in files:
                ...
                print(' Writing to '+ json_summary_file)
                for i in range(0, len(data['CVE_Items'])):
                    ...
                    # Field: CVE-ID, CVE-Description, available for all entries.
...
#                     CVE_OS, CVE_SW, CVE_Ports = fetch_keywords(str(CVE_Desc))
                    ...
                    try: # Field: V2 Score
                        CVSS_V2 = data['CVE_Items'][i]['impact']['baseMetricV2']['impactScore']
                    except KeyError: # Reserved CVEs will have this field value missing
                        CVSS_V2 = 0
                        M_CVSS_V2+=1

                    try: # Field: CVSS Score V3, not all CVEs have CVSS v3 Scores so we keep rest as zero.
                        CVSS_V3 = data['CVE_Items'][i]['impact']['baseMetricV3']['impactScore']
                    except KeyError:
                        CVSS_V3 = 0
                        M_CVSS_V3+=1
...
#                     print(CVE_ID, end=' ')
    ...
    print('\nCWE_Desc:\t'+str(M_CWE_Desc)+'\nCWE_Plat:\t'+str(M_CWE_Plat)+'\nCWE_Af_Res:\t'+str(M_CWE_Af_Res)+
          '\nM_OS:\t\t'+str(M_OS)+'\nM_SW:\t\t'+str(M_SW)+'\nM_Ports:\t'+str(M_Ports))
    print('\n')
    # End 3. Prepare data from downloaded JSON files...

# **************************************************************
# 4. Fetch OS, SW Versions, and Port Numbers from given CVE Description...
...
# Port extractor from string... Final 
...
# # ------------------------------------------
# def fetch_keywords(given_description):
# #     print('Step 4: Reading and summarizing NVD JSON files.')
#     CVE_OS, CVE_SW, CVE_Ports = "N/A", "N/A", "N/A"
#     return CVE_OS, CVE_SW, CVE_Ports
# # ------------------------------------------
...
# Step 3, read JSON files, summarize data and extract keywords...
read_and_summarize()
# Step 4 and 5 will execute based on current CWE-ID and CVE-Description, therefore it will run within Step 3.
